...
// SendChainTransactionA a->b b->c
func SendChainTransactionA() error {
	address1 := ""
	address2 := ""
	address3 := ""
	...
	fmt.Println("transactions hash: " + hexutil.Encode(hash.Bytes()))
...
// SendChainTransactionB a->(bcde)
func SendChainTransactionB() error {
	address1 := ""
	address2 := ""
	address3 := ""
	address4 := ""
	...
	txWithGroupsBuilder := builder.NewCkbTransactionBuilder(network, iterator)
	address := []string{address2, address3, address4, address5}
	for _, a := range address {
		err = txWithGroupsBuilder.AddOutputByAddress(a, 501000000000)
		if err != nil {
			return err
		}
	...
	// sign transactions
	if _, err = signer.GetTransactionSignerInstance(network).SignTransactionByPrivateKeys(txWithGroups, ""); err != nil {
		return err
	}
